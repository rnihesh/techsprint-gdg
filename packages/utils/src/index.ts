import { differenceInDays, differenceInMonths, format, formatDistanceToNow } from 'date-fns';
import type { Issue, IssueStatus, IssueType, Municipality } from '@techsprint/types';
...
export function calculateMunicipalityScore(
  ...
  const penalties: Array<{ issueId: string; daysOpen: number; penalty: number }> = [];
...
export function getBoundsCenter(bounds: { north: number; south: number; east: number; west: number }): {
  lat: number;
  lng: number;
} {
  return {
...
export function generateMunicipalityId(state: string, district: string, name: string): string {
  const sanitize = (str: string) => str.toLowerCase().replace(/[^a-z0-9]/g, '');
  return `MUN-${sanitize(state).slice(0, 3)}-${sanitize(district).slice(0, 3)}-${sanitize(name).slice(0, 6)}`.toUpperCase();
}
...
export function getStatusLabel(status: IssueStatus): string {
  const labels: Record<IssueStatus, string> = {
    OPEN: 'Open',
    RESPONDED: 'Response Pending Verification',
...
export function getVerificationStatus(confidenceScore: number | null): IssueStatus {
  if (confidenceScore === null) return 'RESPONDED';
  return confidenceScore >= SCORING_CONFIG.VERIFICATION_THRESHOLD
    ? 'VERIFIED'
    : 'NEEDS_MANUAL_REVIEW';
}
